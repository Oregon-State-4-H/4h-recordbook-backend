...
func New(logger *zap.SugaredLogger, cfg *config.Config, dbInstance db.Db, upcInstance upc.Upc) (Api, error) {
	...
	router.GET("/project/:projectID/feed", e.getFeeds)
	...
	router.GET("/project/:projectID/feed-purchase", e.getFeedPurchases)
	...
	router.GET("/project/:projectID/:animalID/daily-feed", e.getDailyFeeds)
	...
	router.GET("/project/:projectID/supply", e.getSupplies)
